...
AS (
  ...
  SELECT
    ...
    b.max_tstamp AS page_view_end,
      ...
      TO_CHAR(b.min_tstamp, 'YYYY-MM-DD HH24:MI:SS') AS page_view_time,
      TO_CHAR(b.min_tstamp, 'YYYY-MM-DD HH24:MI') AS page_view_minute,
      TO_CHAR(b.min_tstamp, 'YYYY-MM-DD HH24') AS page_view_hour,
      TO_CHAR(b.min_tstamp, 'YYYY-MM-DD') AS page_view_date,
      TO_CHAR(DATE_TRUNC('week', b.min_tstamp), 'YYYY-MM-DD') AS page_view_week,
      TO_CHAR(b.min_tstamp, 'YYYY-MM') AS page_view_month,
      TO_CHAR(DATE_TRUNC('quarter', b.min_tstamp), 'YYYY-MM') AS page_view_quarter,
      DATE_PART('year',b.min_tstamp)::INTEGER AS page_view_year,

    -- page view: time in the user's local timezone

    b.min_tstamp AS page_view_start_local,
    b.max_tstamp AS page_view_end_local,

      -- example derived dimensions

      TO_CHAR(b.min_tstamp, 'YYYY-MM-DD HH24:MI:SS') AS page_view_local_time,
      TO_CHAR(b.min_tstamp, 'HH24:MI') AS page_view_local_time_of_day,
      DATE_PART('hour',b.min_tstamp)::INTEGER AS page_view_local_hour_of_day,
      TRIM(TO_CHAR(b.min_tstamp, 'd')) AS page_view_local_day_of_week,
      MOD(EXTRACT(DOW FROM b.min_tstamp)::INTEGER - 1 + 7, 7) AS page_view_local_day_of_week_index,
